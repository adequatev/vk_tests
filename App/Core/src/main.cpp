...
#define DEMO_TEXTURE_CACHE_FOLDER ".cache/out_textures_myscene/"
#define fileNameCachedMeshes ".cache/myscene.meshes"
#define fileNameCachedMaterials ".cache/myscene.materials"
#define fileNameCachedHierarchy ".cache/myscene.scene"

#include "Chapter10/Bistro.h"
#include "Chapter10/Skybox.h"
#include "App/VKMesh11Lazy.h"
...
int main()
{
  ...
  const Skybox skyBox(
      ctx, "data/farm_field_puresky_2k_prefilter.ktx", "data/farm_field_puresky_2k_irradiance.ktx", kOffscreenFormat, app.getDepthFormat(),
      kNumSamples);
  ...
  const VKPipeline11 pipelineOpaque(
      ...
      loadShaderModule(ctx, "App/Core/src/main.vert"), loadShaderModule(ctx, "App/Core/src/opaque.frag"));
  const VKPipeline11 pipelineTransparent(
      ctx, meshData.streams, kOffscreenFormat, app.getDepthFormat(), kNumSamples,
      loadShaderModule(ctx, "App/Core/src/main.vert"), loadShaderModule(ctx, "App/Core/src/transparent.frag"));
  const VKPipeline11 pipelineShadow(
      ctx, meshData.streams, lvk::Format_Invalid, ctx->getFormat(texShadowMap), 1,
      loadShaderModule(ctx, "App/03_DirectionalShadows/src/shadow.vert"),
      loadShaderModule(ctx, "App/03_DirectionalShadows/src/shadow.frag"));

  lvk::Holder<lvk::ShaderModuleHandle> vertOIT       = loadShaderModule(ctx, "data/shaders/QuadFlip.vert");
  lvk::Holder<lvk::ShaderModuleHandle> fragOIT       = loadShaderModule(ctx, "App/04_OIT/src/oit.frag");
  ...
  lvk::Holder<lvk::ShaderModuleHandle> compCulling        = loadShaderModule(ctx, "App/02_CullingGPU/src/FrustumCulling.comp");
  ...
  app.run([&](uint32_t width, uint32_t height, float aspectRatio, float deltaSeconds) {
    ...
    // axis aligned bound boxes -> light space
    ...
    lvk::ICommandBuffer& buf = ctx->acquireCommandBuffer();
    {
      ...
      if (cullingMode == CullingMode_None) {
        ...
        numVisibleMeshes =
            static_cast<uint32_t>(meshesTransparent.drawCommands_.size());
      ...
      // render UI
      {
        ...
        ImGui::SetNextWindowSizeConstraints(ImVec2(-1, 0), ImVec2(-1, v->WorkSize.y - 270));
        ImGui::Begin("scaffolding test ui", nullptr, ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_AlwaysAutoResize);
        const float indentSize = 24.0f;
        ...
        ImGui::Separator();
        if (ImGui::CollapsingHeader("Occlusion culliing")) {
          ImGui::Indent(indentSize);
          ImGui::RadioButton("Turn OFF", &cullingMode, CullingMode_None);
          //ImGui::RadioButton("CPU  (C)", &cullingMode, CullingMode_CPU);
          ImGui::RadioButton("Turn ON", &cullingMode, CullingMode_GPU);
          ImGui::Unindent(indentSize);
          ImGui::Checkbox("Stop viewer updates", &freezeCullingView);
          ImGui::Separator();
          //ImGui::Text("Visible : %i", numVisibleMeshes);
          //ImGui::Separator();
        }
        ...
        //  ImGui::Separator();
        //}
        if (ImGui::CollapsingHeader("Global direct lighting")) {
          ImGui::Text("\tDepth settings:");
          ImGui::Indent(indentSize);
          ImGui::SliderFloat("\tConstant", &light.depthBiasConst, -5.0f, 5.0f);
          ImGui::SliderFloat("\tSlope", &light.depthBiasSlope, -5.0f, 5.0f);
          ImGui::Unindent(indentSize);
          ImGui::Separator();
          ImGui::Text("\tLight rotation:");
          ImGui::Indent(indentSize);
          ImGui::SliderFloat("\tY-axis", &light.theta, -180.0f, +180.0f);
          ImGui::SliderFloat("\tZ-axis", &light.phi, -85.0f, +85.0f);
          ImGui::Unindent(indentSize);
          //ImGui::Image(texShadowMap.index(), ImVec2(512, 512));
        }
        if (ImGui::CollapsingHeader("Screen Space Ambient Occlusion")) {
          ImGui::Indent(indentSize);
          ImGui::Checkbox("\tTurn on", &ssaoEnable);
          ImGui::BeginDisabled(!ssaoEnable);
          ImGui::Checkbox("\tEnable Blur effect", &ssaoEnableBlur);
          ImGui::BeginDisabled(!ssaoEnableBlur);
          ImGui::SliderFloat("\tBlur threshold", &ssaoDepthThreshold, 0.0f, 50.0f);
          ImGui::SliderInt("\tBlur iterations", &ssaoNumBlurPasses, 1, 5);
          ImGui::EndDisabled();
          ImGui::SliderFloat("\tScale", &pcCombineSSAO.scale, 0.0f, 2.0f);
          ImGui::SliderFloat("\tBias", &pcCombineSSAO.bias, 0.0f, 0.3f);
          ImGui::SliderFloat("\tRadius", &pcSSAO.radius, 0.001f, 0.02f);
          //ImGui::SliderFloat("attenuation scale", &pcSSAO.attScale, 0.5f, 1.5f);
        ...
        if (ImGui::CollapsingHeader("HDR")) {
          ...
          ImGui::SliderFloat("\tExposure", &pcHDR.exposure, 0.1f, 100.0f);
          //ImGui::SliderFloat("Adaptation speed", &hdrAdaptationSpeed, 1.0f, 10.0f);
          ImGui::Checkbox("\tEnable Bloom effect", &hdrEnableBloom);
          ...
          ImGui::RadioButton("\tTurn [off]", &pcHDR.drawMode, ToneMapping_None);
          ...
          //  ImGui::Unindent(indentSize);
          //}
          ImGui::RadioButton("\tTurn [on]", &pcHDR.drawMode, ToneMapping_KhronosPBR);
