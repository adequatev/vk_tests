...
void loadBistro(MeshData& meshData, Scene& scene) {
  if (!isMeshDataValid(fileNameCachedMeshes) || !isMeshHierarchyValid(fileNameCachedHierarchy) ||
    ...
    loadMeshFile("deps/src/full_gameready_city_buildings_retextured/scene.gltf", meshData_Exterior, ourScene_Exterior, false);
    loadMeshFile("deps/src/Avocado/Avocado.gltf", meshData_Interior, ourScene_Interior, false);
